// im-blog-lang-viewer v0.0.0

import * as bl from "blog-lang";
import { el, im, ImCache, imdom } from "im-js";
import { cssVars, DisplayType, imui, BLOCK, CENTER, COL, INLINE, LEFT, NA, PX, ROW } from "im-ui";
...
export function imRenderBlogLangMarkup(c: ImCache, markup: string, markupVersion: number) {
	...
	imRenderBlogLangBlocks(c, s.blogpost.blocks);
}

export function imRenderBlogLangBlocks(c: ImCache, blocks: bl.Block[]) {
	imBegin(c); imui.Padding(c, 15, PX, 15, PX, 15, PX, 15, PX); {
		imRenderBlocksInternal(c, blocks, 0);
	} imEnd(c);
}

export function imRenderBlocksInternal(c: ImCache, blocks: bl.Block[], depth: number) {
...
export function imRenderBlogpostBlock(c: ImCache, block: bl.Block, depth = 0) {
	imBegin(c); {
